...
SFAsset::SFAsset(SFASSETTYPE type, std::shared_ptr<SFWindow> window): type(type), sf_window(window) {
  ...
  case SFASSET_COIN:
    ...
    break;
  case SFASSET_WALL:
    sprite = IMG_LoadTexture(sf_window->getRenderer(), "assets/wall.jpeg");
    break;
  case SFASSET_WALL2:
    sprite = IMG_LoadTexture(sf_window->getRenderer(), "assets/wall2.jpeg");
    break;
  case SFASSET_BACKGROUND:
    sprite = IMG_LoadTexture(sf_window->getRenderer(), "assets/background.jpeg");
    break;
  }
